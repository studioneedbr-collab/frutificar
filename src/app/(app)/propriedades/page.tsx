...
/* DEV PREVIEW — sem banco. Estado em memória das propriedades de um produtor. */

type Status = 'Saudável' | 'Atenção' | 'Pousio'
...
const initialProperties: Property[] = [
  {
    ...
    altitude: '980 m',
    talhoesList: [
      { id: 1, name: 'Talhão A1', cultura: 'Café Catuaí', area: '24 ha', status: 'Saudável' },
      { id: 2, name: 'Talhão A2', cultura: 'Café Mundo Novo', area: '18 ha', status: 'Atenção' },
      { id: 3, name: 'Talhão B1', cultura: 'Café Bourbon', area: '22 ha', status: 'Saudável' },
      { id: 4, name: 'Várzea', cultura: 'Pousio', area: '20 ha', status: 'Pousio' },
    ],
  },
  {
    id: 2,
    ...
    cultura: 'Café + Milho',
    talhoesList: [
      { id: 1, name: 'Talhão 1', cultura: 'Café Catuaí', area: '16 ha', status: 'Saudável' },
      { id: 2, name: 'Talhão 2', cultura: 'Milho', area: '12 ha', status: 'Atenção' },
    ],
  },
]
...
export default function PropriedadesPage() {
  ...
  const [nextPropId, setNextPropId] = useState(initialProperties.length + 1)

  const [addOpen, setAddOpen] = useState(false)

  // Talhão modal: target property id
  const [talhaoTargetId, setTalhaoTargetId] = useState<number | null>(null)

  // Confirm dialogs
  const [removeTalhao, setRemoveTalhao] = useState<{ propId: number; talhaoId: number } | null>(null)
  const [removeProp, setRemoveProp] = useState<number | null>(null)
  ...
  function handleAddTalhao(e: React.FormEvent<HTMLFormElement>) {
    e.preventDefault()
    ...
    const cultura = String(data.get('cultura') ?? '').trim()
    const area = String(data.get('area') ?? '').trim()
    const status = (String(data.get('status') ?? 'Saudável') as Status)

    setProperties((prev) =>
      prev.map((p) => {
        if (p.id !== talhaoTargetId) return p
        const nextId = p.talhoesList.reduce((max, t) => Math.max(max, t.id), 0) + 1
        const list = [
          ...p.talhoesList,
          { id: nextId, name, cultura: cultura || '—', area: area ? `${area} ha` : '—', status },
        ]
  ...
  function confirmRemoveProperty() {
    ...
    setProperties((prev) => prev.filter((p) => p.id !== removeProp))
    setRemoveProp(null)
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-6 pb-4">
          ...
          <div className="flex flex-col md:flex-row gap-5 md:gap-6">
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="flex flex-wrap items-center gap-2.5 mt-5">
                ...
                <button
                  onClick={() => setTalhaoTargetId(primary.id)}
      ...
      {rest.map((p, i) => (
        ...
        >
          <div className="flex items-center gap-4">
            ...
            <button
              onClick={() => setTalhaoTargetId(p.id)}
      ...
      {/* Novo talhão */}
      <Dialog open={talhaoTargetId !== null} onOpenChange={(o) => !o && setTalhaoTargetId(null)}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2" style={{ fontFamily: 'var(--font-heading)', color: 'var(--color-frutificar-deep)' }}>
          ...
          <form onSubmit={handleAddTalhao} className="space-y-3.5">
            ...
            <div className="grid grid-cols-2 gap-3">
              ...
              <div>
                ...
                <select name="status" defaultValue="Saudável"
                  className={inputClass} style={inputStyle}>
            ...
            <DialogFooter className="gap-2 sm:gap-2 pt-1">
              <button type="button" onClick={() => setTalhaoTargetId(null)}
                className="px-4 py-2.5 rounded-xl font-semibold text-sm transition-colors hover:bg-[oklch(0.96_0.01_144)]"
                style={{ color: 'oklch(0.45 0.04 144)', border: '1px solid oklch(0.91 0.01 144)' }}>
                Cancelar
              </button>
