...
/* Dados mock de um produtor Gold (campos sem persistência ficam locais). */
const user = {
  phone: '(34) 99812-3344',
  city: 'Patrocínio/MG',
...
export function PerfilView({
  ...
  /* ── Dados pessoais (Nome/E-mail persistem; demais ficam locais) ── */
  const [name, setName] = useState(initialName)
  const [email, setEmail] = useState(initialEmail)
  const [phone, setPhone] = useState(user.phone)
  const [city, setCity] = useState(user.city)
  const [property, setProperty] = useState(user.property)
  const [cultura, setCultura] = useState(user.cultura)

  const initials =
    name.trim().split(/\s+/).map((p) => p[0] ?? '').join('').slice(0, 2).toUpperCase() || user.initials
  ...
  async function handleChangePassword(e: React.FormEvent<HTMLFormElement>) {
    ...
    const atual = String(data.get('atual') ?? '')
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            <label className="text-xs font-semibold flex items-center gap-1.5 mb-1.5" style={{ color: 'var(--color-frutificar-deep)' }}>
              <User size={13} style={{ color: 'oklch(0.55 0.04 144)' }} /> Nome completo
            </label>
            <input
              type="text"
              value={name}
          ...
          <div>
            <label className="text-xs font-semibold flex items-center gap-1.5 mb-1.5" style={{ color: 'var(--color-frutificar-deep)' }}>
              <Mail size={13} style={{ color: 'oklch(0.55 0.04 144)' }} /> E-mail
            </label>
            <input
              type="email"
              value={email}
          ...
          <div>
            <label className="text-xs font-semibold flex items-center gap-1.5 mb-1.5" style={{ color: 'var(--color-frutificar-deep)' }}>
              <Phone size={13} style={{ color: 'oklch(0.55 0.04 144)' }} /> Telefone
            </label>
            <input
              type="tel"
              value={phone}
          ...
          <div>
            <label className="text-xs font-semibold flex items-center gap-1.5 mb-1.5" style={{ color: 'var(--color-frutificar-deep)' }}>
              <MapPin size={13} style={{ color: 'oklch(0.55 0.04 144)' }} /> Cidade/UF
            </label>
            <input
              type="text"
              value={city}
          ...
          <div>
            <label className="text-xs font-semibold flex items-center gap-1.5 mb-1.5" style={{ color: 'var(--color-frutificar-deep)' }}>
              <Sprout size={13} style={{ color: 'oklch(0.55 0.04 144)' }} /> Nome da propriedade
            </label>
            <input
              type="text"
              value={property}
          ...
          <div>
            <label className="text-xs font-semibold flex items-center gap-1.5 mb-1.5" style={{ color: 'var(--color-frutificar-deep)' }}>
              <Sprout size={13} style={{ color: 'oklch(0.55 0.04 144)' }} /> Cultura principal
            </label>
            <input
              type="text"
              value={cultura}
