...
type ServiceTarget = { name: string; priceLabel?: string }
...
export function ServicosView({ preview }: { preview: boolean }) {
  ...
  function openRequest(target: ServiceTarget) {
    setTalhao(talhoes[0])
    setData('')
  ...
  async function handleRequestSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    const priceLabel = requestTarget?.priceLabel
    const obs = String(new FormData(e.currentTarget).get('observacoes') ?? '').trim()
    const description =
      [talhao, data ? `Data desejada: ${data}` : '', priceLabel ?? '', obs]
        .filter(Boolean)
